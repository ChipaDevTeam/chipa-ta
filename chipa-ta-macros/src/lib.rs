use std::{collections::HashMap, fs::read_to_string, sync::{Arc, RwLock}};

use darling::{ast::{Data, NestedMeta}, util::Ignored, Error, FromDeriveInput, FromMeta, FromVariant};
use lazy_static::lazy_static;
use proc_macro::TokenStream;
use proc_macro2::{Span, TokenStream as TokenStream2};
use quote::{quote, ToTokens};
use syn::{
    parse_macro_input, Attribute, DeriveInput, Expr, FnArg, Ident, ItemTrait, Lit, Meta, Pat, ExprLit, Signature, TraitItemFn
};
lazy_static! {
    static ref TRAIT_METHODS: Arc<RwLock<HashMap<String, Vec<String>>>> = Arc::new(RwLock::new(HashMap::new()));
}
    ...
    fn from_list(items: &[NestedMeta]) -> darling::Result<Self> {
        ...
        if items.len() == 1 {
            if let NestedMeta::Meta(Meta::NameValue(nv)) = &items[0] {
                let trait_method = nv.path.get_ident()
        ...
        Err(Error::custom("Expected method(trait_method = \"enum_method\")"))
    }
}
...
impl AutoImplOpts {
    ...
    fn implement_trait(&self, trait_name: &Ident, methods: Vec<String>, maps: &HashMap<Ident, Ident>) -> syn::Result<TokenStream2> {
        ...
        let methods_signatures = methods.iter().map(|s| syn::parse_str::<TraitItemFn>(s).map(|f| (f.sig, f.attrs)))
            .collect::<Result<Vec<(Signature, Vec<Attribute>)>, syn::Error>>()?;
        let methods_impl = methods_signatures.iter().map(|(s, attrs)| self.implement_method(s, attrs.clone(), maps)).collect::<syn::Result<Vec<TokenStream2>>>()?;
    ...
    fn implement_method(&self, signature: &Signature, attrs: Vec<Attribute>, maps: &HashMap<Ident, Ident>) -> syn::Result<TokenStream2> {
        ...
        let param_names: Vec<TokenStream2> = inputs.iter().filter_map(|input| {
        ...
        // Generate match arms
        let match_arms: Vec<TokenStream2> = fields.iter().map(|field| {
    ...
    fn load_traits_from_file(&self) -> syn::Result<HashMap<String, Vec<String>>> {
        if let Some(path) = &self.path {
            let content = read_to_string(path).map_err(|e| syn::Error::new(Span::call_site(), format!("Failed to read file '{}': {}", path, e)))?;
            let mut methods: HashMap<String, Vec<String>> = HashMap::new();
            let file = syn::parse_file(&content)?;
            for item in file.items {
                if let syn::Item::Trait(trait_item) = item {
                    let trait_name = trait_item.ident.to_string();
                    let trait_methods: Vec<String> = trait_item.items.iter().filter_map(|item| {
            ...
            Err(syn::Error::new(Span::call_site(), "Path to traits file is not specified"))
        }
    }
    ...
    fn create_impls(&self) -> syn::Result<TokenStream2> {
        ...
        let traits_and_methods = match &self.path {
            Some(_) => {
                self.load_traits_from_file()?
            }
            None => {
                TRAIT_METHODS.read().unwrap().clone()
            }
        };

        let trait_impls = self.traits.iter().map(|trait_name| {
            let methods = traits_and_methods.get(&trait_name.to_string())
                .ok_or_else(|| syn::Error::new(Span::call_site(), format!("Trait '{}' not registered", trait_name)))?;
...
pub fn register_trait(input: TokenStream) -> TokenStream {
    ...
    let trait_methods_str: Vec<String> = input_trait.items.iter().filter_map(|item| {
        if let syn::TraitItem::Fn(method) = item {
            Some(method.to_token_stream().to_string())
        } else {
            None
